...
`.js` / `.ts` / `.py` / `.sh` / `.env` 先切出註解再套規則:

- 程式碼本體與字串字面值一律不掃,含中文的字串字面值也不掃
  - 字串是給人看的訊息,但句長與標點的判準與註解內文不同
  - 述句結尾的半形分號因此不再被當中文標點誤判
- 註解標記與 JSDoc 續行的 `*` 遮掉,註解內以 `-` 開頭的條目落在行首,LIST 三規則照 markdown 處理
- `paren-supplement` 只在 JSDoc 描述首行報
  - 程式註解內文的括號多半是合法 gloss 或 cross-ref
- `@param` / `@returns` 的型別與參數名段不計句長

表列之外的副檔名照 markdown 走。
